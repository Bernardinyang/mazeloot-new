...
export const adminRoutes = [
  {
    ...
    children: [
      ...
      { path: 'health', name: 'admin-health', component: () => import('@/admin/views/health/Index.vue'), meta: { breadcrumb: 'Health', requiresSuperAdmin: true, ...a('Health') }, beforeEnter: (to, from, next) => { if (!useUserStore().isSuperAdmin) next({ name: 'admin-dashboard' }); else next() } },
      ...
      { path: 'system', name: 'admin-system', component: () => import('@/admin/views/system/Index.vue'), meta: { breadcrumb: 'System', requiresSuperAdmin: true, ...a('System') }, beforeEnter: (to, from, next) => { if (!useUserStore().isSuperAdmin) next({ name: 'admin-dashboard' }); else next() } },
