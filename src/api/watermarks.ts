 ...
 */
const initializeMockData = (): Watermark[] => {
  const existing = storage.get<Watermark[]>(WATERMARKS_STORAGE_KEY)
  ...
  const mockWatermarks: Watermark[] = [
    {
      id: generateUUID(),
      name: 'Company Logo',
      type: 'image',
 ...
 */
const getAllWatermarks = (): Watermark[] => {
  const watermarks = storage.get<Watermark[]>(WATERMARKS_STORAGE_KEY)
 ...
 */
const saveWatermarks = (watermarks: Watermark[]): void => {
