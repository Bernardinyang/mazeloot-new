          ...
          <CardHeader v-if="title || description">
            <div v-if="title || description" class="flex items-center justify-between">
              <div>
                ...
                <CardDescription v-if="description" :class="descriptionClasses">{{ description }}</CardDescription>
              </div>
...
import CardFooter from '@/components/shadcn/CardFooter.vue'
...
const baseCardClasses = computed(() => `${theme.bgCard} backdrop-blur-md ${theme.borderCard} ${theme.transition} hover:scale-[1.01] ${theme.shadowCard}`)
const baseTitleClasses = computed(() => `${theme.textPrimary} text-sm font-semibold uppercase tracking-wide`)
