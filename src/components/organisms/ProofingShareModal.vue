...
const shareLink = computed(() => {
  ...
  // Format: /p/:projectId/proofing?proofingId=:proofingId
  const baseUrl = window.location.origin

  if (props.projectId) {
    // Use project-based public route
    return `${baseUrl}/p/${props.projectId}/proofing?proofingId=${props.proofingId}`
  } else {
    // Fallback: if no project ID, use proofing ID directly
    return `${baseUrl}/p/standalone/proofing?proofingId=${props.proofingId}`
  }
})
