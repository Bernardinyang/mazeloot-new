  ...
  <div class="relative" ref="pickerRef">
    <div class="flex items-center gap-2">
      <button
        ...
        "
        :style="
          displayColor === 'transparent' || !displayColor || displayColor === ''
            ? {
                background:
                  'linear-gradient(45deg, #ccc 25%, transparent 25%), linear-gradient(-45deg, #ccc 25%, transparent 25%), linear-gradient(45deg, transparent 75%, #ccc 75%), linear-gradient(-45deg, transparent 75%, #ccc 75%); background-size: 8px 8px; background-position: 0 0, 0 4px, 4px -4px, -4px 0px;',
              }
    ...
    >
      <!-- Transparent Option (if allowed) -->
      <div v-if="allowTransparent" class="mb-4">
        <label class="text-sm font-medium mb-2 block">Background</label>
        <div class="grid grid-cols-8 gap-2">
          <button
            type="button"
            class="h-8 w-8 rounded-md border-2 cursor-pointer hover:scale-110 transition-transform flex items-center justify-center"
            :class="
              displayColor === 'transparent' || !displayColor || displayColor === ''
                ? 'border-primary'
                : 'border-input'
          ...
          >
            <span
              v-if="displayColor === 'transparent' || !displayColor || displayColor === ''"
              class="text-xs"
              >✓</span
            >
          </button>
      ...
      <div class="mb-4">
        <label class="text-sm font-medium mb-2 block">{{
          allowTransparent ? 'Colors' : 'Preset Colors'
        }}</label>
        <div class="grid grid-cols-8 gap-2">
          <button
...
const displayColor = computed({
  get: () => {
    if (props.modelValue === null || props.modelValue === undefined || props.modelValue === '') {
      return props.allowTransparent ? 'transparent' : '#000000'
    }
    return internalValue.value || props.modelValue || '#000000'
  },
  set: (value: string) => {
    internalValue.value = value
    // If transparent is selected and allowTransparent is true, emit null
    if (props.allowTransparent && (value === 'transparent' || value === '')) {
...
const selectColor = (color: string) => {
  if (color === 'transparent' && props.allowTransparent) {
    displayColor.value = ''
    emit('update:modelValue', null)
  } else {
...
const handleBlur = () => {
  // Ensure valid hex color on blur (unless transparent is allowed and value is empty)
  if (
    props.allowTransparent &&
    (displayColor.value === '' || displayColor.value === 'transparent')
  ) {
    emit('update:modelValue', null)
  } else if (!displayColor.value.match(/^#[0-9A-Fa-f]{6}$/)) {
