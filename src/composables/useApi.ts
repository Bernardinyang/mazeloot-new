...
export interface UseApiOptions {
  key?: string // Unique key for cancellation and caching
  ...
  const request = async <T = any>(
    ...
    try {
      const makeRequest = async (): Promise<ApiResponse<T>> => {
        switch (method) {
          case 'get':
            return await apiClient.get<T>(endpoint, { ...callOptions, signal })
          case 'post':
            return await apiClient.post<T>(endpoint, data, { ...callOptions, signal })
          case 'put':
            return await apiClient.put<T>(endpoint, data, { ...callOptions, signal })
          case 'patch':
            return await apiClient.patch<T>(endpoint, data, { ...callOptions, signal })
          case 'delete':
            return await apiClient.delete<T>(endpoint, { ...callOptions, signal })
        }
      }
