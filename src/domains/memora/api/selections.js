...
export function useSelectionsApi() {
  ...
  const createMediaSet = async (selectionId, data) => {
    try {
      const { normalizeSelectionLimit } = await import('@/domains/memora/utils/selectionLimit')
      const createData = {
        name: data.name,
   ...
   * @param {string} mediaId - Media ID
   */
  const toggleRecommended = async (selectionId, setId, mediaId) => {
    try {
      const response = await apiClient.post(
        `/v1/selections/${selectionId}/sets/${setId}/media/${mediaId}/recommend`
      )
      return response.data
    } catch (error) {
      throw parseError(error)
