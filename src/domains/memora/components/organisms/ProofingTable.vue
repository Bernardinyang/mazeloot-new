<template>
  <DataTable
    ...
    searchable
    search-placeholder="Search proofing…"
  ...
  >
    ...
    <template #cell-icon="{ item }">
      <div :class="theme.bgCardSolid" class="w-10 h-10 rounded overflow-hidden">
        ...
        >
          <component :is="getItemIcon(item) || Eye" :class="theme.textTertiary" class="h-5 w-5" />
        </div>
      </div>
    </template>
...
import { Eye, Info, Lock, MoreVertical, Pencil, Star, Trash2 } from '@/shared/utils/lucideAnimated'
import {
  DropdownMenu,
  DropdownMenuContent,
  DropdownMenuItem,
...
const props = defineProps({
  ...
  getImage: {
    type: Function,
    default: item =>
      item.coverPhotoUrl || item.cover_photo_url || item.thumbnail || item.image || null,
  },
  getPreviewImages: {
    type: Function,
