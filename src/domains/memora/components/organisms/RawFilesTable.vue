<template>
  <DataTable
    :columns="columns"
    searchable
    search-placeholder="Search raw files…"
  ...
  >
    ...
    <template #cell-password="{ item }">
      <div class="flex items-center gap-2" @click.stop>
        <PasswordCell v-if="getItemHasPassword(item) && getItemPassword(item)" :password="getItemPassword(item)" />
      </div>
    </template>
...
import { CheckSquare, Info, Lock, MoreVertical, Pencil, Star, Trash2 } from '@/shared/utils/lucideAnimated'
import {
  DropdownMenu,
  DropdownMenuContent,
  DropdownMenuItem,
...
const props = defineProps({
  ...
  getHasPassword: {
    type: Function,
    default: item => item.hasPassword || false,
  },
  getMediaCount: {
    type: Function,
...
const getItemHasPassword = item => props.getHasPassword(item)
...
const handlePasswordToggle = (item, enabled) => {
  emit('toggle-password', item, enabled)
}
