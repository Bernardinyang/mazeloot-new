    ...
    <DialogContent :class="[theme.bgCard, theme.borderSecondary]" class="sm:max-w-[520px]">
      <DialogHeader>
        ...
        <DialogDescription :class="theme.textSecondary" class="text-sm mt-1">
          Share this selection with your client via link, QR code, WhatsApp, or email.
        </DialogDescription>
      </DialogHeader>
        ...
        <div class="space-y-2">
          ...
          >
            ...
            <p :class="theme.textSecondary" class="text-xs text-center max-w-xs">
              Scan this QR code to access the selection
            </p>
          </div>
        </div>
...
import { publicSelectionUrl } from '@/shared/utils/memoraPublicUrls'
...
const shareLink = computed(() => {
  ...
  const domain = props.brandingDomain || props.projectId || props.selectionId
  const path = publicSelectionUrl(domain, props.selectionId)
...
const handleShareWhatsApp = () => {
  const message = encodeURIComponent(
    `Check out this selection: ${props.selectionName}\n${shareLink.value}`
  )
...
const handleShareEmail = () => {
  const subject = encodeURIComponent(`Selection: ${props.selectionName}`)
  const body = encodeURIComponent(`Check out this selection:\n\n${shareLink.value}`)
