...
import { validateUploadFile } from '@/shared/utils/media/validateUploadFile'
import { splitDuplicateUploadFiles } from '@/shared/utils/media/splitDuplicateUploadFiles'
import { filterValidUploadFiles, filterValidRawFiles, VALID_RAW_FILES_MIME_TYPES } from '@/shared/utils/media/filterValidUploadFiles'
import { VALID_UPLOAD_MIME_TYPES } from '@/shared/utils/media/filterValidUploadFiles'
...
export function useMediaUpload(options = {}) {
  const {
    ...
    batchSize = 1,
    maxRetries = 3,
    contextType = 'collection', // collection, selection, proofing, or raw_files
    useBackgroundUploads = true,
  } = options

  // Set default allowedTypes based on contextType if not provided
  const defaultAllowedTypes = contextType === 'raw_files' ? VALID_RAW_FILES_MIME_TYPES : VALID_UPLOAD_MIME_TYPES
  ...
  const processFiles = async (files, setId = null) => {
    ...
    // Filter valid files
    const validFiles = contextType === 'raw_files' 
      ? filterValidRawFiles(Array.from(files))
      : filterValidUploadFiles(Array.from(files))
    ...
    try {
      ...
      uploadResult = await retryWithBackoff(async () => {
        return await apiClient.uploadWithProgress(uploadEndpoint, file, {
          signal,
          phase: contextType, // Pass phase for phase-specific validation
          purpose: contextType ? `memora:${contextType}` : 'memora',
