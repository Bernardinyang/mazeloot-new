    ...
    <DialogContent :class="[theme.bgCard, theme.borderSecondary]" class="sm:max-w-[500px]">
      <DialogHeader>
        ...
        <DialogDescription :class="theme.textSecondary" class="text-sm mt-1">
          ...
          {{ context === 'proofing' ? 'proofing' : context === 'raw_files' ? 'raw files' : 'selection' }}.
        </DialogDescription>
      </DialogHeader>
...
const props = defineProps({
  ...
  context: {
    type: String,
    default: 'selection', // 'selection', 'proofing', or 'raw_files'
    validator: value => ['selection', 'proofing', 'raw_files'].includes(value),
  },
  rawFilesId: {
    type: String,
    default: null,
...
const handleGoToSettings = () => {
  ...
  } else if (props.context === 'raw_files') {
    router.push({
      name: 'rawFilesDetail',
      params: { id: props.rawFilesId },
      query: { tab: 'settings', section: 'general' },
    })
  } else {
    router.push({
...
const handleSave = async () => {
  ...
  try {
    ...
    if (props.context === 'proofing') {
      ...
      updatedItem = await rawFilesApi.updateRawFiles(props.rawFilesId, {
        allowedEmails: uniqueEmails,
      })
    } else {
