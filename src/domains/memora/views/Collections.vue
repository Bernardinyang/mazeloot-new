...
import EditCollectionDialog from '@/shared/components/organisms/EditCollectionDialog.vue'
...
import MoveCollectionModal from '@/domains/memora/modals/MoveCollectionModal.vue'
import DeleteConfirmationModal from '@/domains/memora/modals/DeleteConfirmationModal.vue'
import CollectionShareModal from '@/domains/memora/modals/CollectionShareModal.vue'
import CollectionDetailSidebar from '@/shared/components/organisms/CollectionDetailSidebar.vue'
...
const handleCopyLink = async collection => {
  ...
  try {
    ...
    const memoraPreference = preferences.data?.find(p => p.product?.slug === 'memora')
    domain = memoraPreference?.domain
  } catch (error) {
    console.error('Failed to get domain:', error)
  }
  
  // Use new format: /memora/{domain}/collections/{id}
  const domainOrUsername = domain || 'default' // Fallback if no domain
  const link = `${window.location.origin}/memora/${domainOrUsername}/collections/${collectionId}`
...
const handlePreviewCollection = async collection => {
  ...
  // Use new format: /memora/{domain}/collections/{id}
  const previewUrl = `${window.location.origin}/memora/${domain}/collections/${collectionId}`
