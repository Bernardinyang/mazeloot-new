...
const userId = computed(() => {
  // Support both new format (domainOrUsername) and legacy format (userId query/param)
  return route.params.domainOrUsername || route.query.userId || route.params.userId || null
})
...
const loadHomepageData = async () => {
  ...
  try {
    ...
    if (isOwnerPreview) {
      try {
        const { useSocialLinksApi } = await import('@/domains/memora/api/socialLinks')
