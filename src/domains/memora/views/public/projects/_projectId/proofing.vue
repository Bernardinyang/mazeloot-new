...
const handleSubmitEmail = async () => {
  ...
  try {
    const proofingId = route.params.id || route.query.proofingId
    if (!proofingId) {
...
const loadProofing = async () => {
  ...
  try {
    // Get proofing ID from route params (route is /memora/:domainOrUsername/proofing/:id)
    // Fallback to query param for legacy routes
    const proofingId = route.params.id || route.query.proofingId
    ...
    try {
      ...
      // Handle 404 errors - proofing not found
      ...
      if (
        errorCode === 'NOT_FOUND' ||
        errorMessage.toLowerCase().includes('not found') ||
        errorMessage.toLowerCase().includes('no query results')
      ) {
        throw new Error('Proofing not found. Please check the URL and try again.')
      }
    ...
    logger.error('Error loading proofing:', error)
...
const handleLogout = () => {
  const proofingId = route.params.id || route.query.proofingId || route.params.projectId
  if (proofingId) {
