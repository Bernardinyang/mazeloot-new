<template>
  ...
  <RawFileLayout
    ...
    :on-reset-limit="handleResetRawFileLimit"
    :overall-progress="overallProgress"
    :selected-count="selectedCountAcrossRawFile"
    :rawFile="rawFile"
  ...
  >
    <template #content>
      <input
        ref="fileInputRef"
        :disabled="rawFile?.status === 'completed'"
        accept="*/*"
        class="hidden"
        multiple
        type="file"
      ...
      >
        ...
        <div v-else class="p-4 sm:p-6 md:p-8">
          ...
          <BulkActionsBar
            v-if="selectedSetId"
            :disable-actions="rawFile?.status === 'completed'"
            ...
            :hide-watermark="true"
            :can-preview="canPreviewSelected"
          ...
          <div v-else-if="selectedSetId && sortedMediaItems.length > 0" class="mb-8">
            ...
            >
              <MediaGridItemCard
                ...
                :show-management-actions="rawFile?.status !== 'completed'"
                :was-selected-on-completion="
                  rawFile?.status === 'completed' ? item.isSelected : false
                "
                @delete="handleDeleteMedia(item)"
                @download="handleDownloadMediaWithPin(item)"
                @open="handleOpenMedia(item)"
                @rename="handleRenameMedia(item)"
                @replace="handleReplacePhoto(item)"
                @watermark="handleWatermarkMedia(item)"
                @toggle-selection="handleToggleMediaRawFile(getItemId(item))"
          ...
          >
            <EmptyState
              ...
              message="No sets in this rawFile"
          ...
          <MediaUploadDropzone
            ...
            @drop="handleDrop"
          />
          <!-- Show empty state for completed rawFiles or when no media -->
          <div
            v-else-if="sortedMediaItems.length === 0 && !isLoadingMedia"
            class="text-center py-16"
          >
            <p :class="theme.textSecondary" class="text-lg mb-4">No media in this rawFile yet</p>
            <p :class="theme.textTertiary" class="text-sm">
              <span v-if="rawFile && rawFile.status === 'completed'">
                This rawFile has been completed.
              </span>
      ...
      <CreateEditMediaSetModal
        ...
        v-model:rawFileLimit="mediaSetsSidebar.newSetRawFileLimit"
      ...
      <!-- RawFile Limit Modal -->
      <RawFileLimitModal
        v-model="showRawFileLimitModal"
        :current-limit="rawFile?.rawFileLimit"
        :is-saving="isSavingRawFileLimit"
        @cancel="handleCancelRawFileLimit"
        @save="handleSaveRawFileLimit"
      />
      ...
      <MediaLightbox
        ...
        @download="handleDownloadMediaWithPin"
      ...
      <MoveCopyModal
        ...
        :current-collection-id="rawFile?.id || ''"
        :current-collection-name="rawFile?.name || ''"
        :is-loading-sets="isLoadingTargetSets"
        :is-moving="isMovingMedia"
        :selected-count="selectedMediaIds.size"
        :target-collection-id="targetRawFileId"
        :target-collection-sets="targetRawFileSets"
        :target-set-id="targetSetId"
        context="rawFile"
        @cancel="handleCancelMoveCopy"
        @confirm="handleConfirmMoveCopy"
        @rawFile-change="handleTargetRawFileChange"
      ...
      <!-- Download PIN Modal -->
      <Dialog v-if="showDownloadPinModal && requiresDownloadPin && !isDownloadPinVerified" :open="showDownloadPinModal" @update:open="(val) => { if (!val) { showDownloadPinModal = false; downloadPinInput = ''; downloadPinError = '' } }">
        <DialogContent class="sm:max-w-[440px] bg-white dark:bg-gray-900 border border-gray-200 dark:border-gray-700 shadow-2xl">
          <DialogHeader class="text-center pb-2">
            ...
            <DialogDescription class="text-sm mt-2 text-gray-600 dark:text-gray-300">
              Please enter the 4-digit download PIN to download files from this raw file
            </DialogDescription>
          </DialogHeader>
...
import { useMemoraFeatures } from '@/domains/memora/composables/useMemoraFeatures'
  import RawFileSettingsGeneral from '@/domains/memora/views/raw-files/settings/General.vue'
...
import RawFileLimitModal from '@/domains/memora/components/organisms/RawFileLimitModal.vue'
...
import { AlertCircle } from '@/shared/utils/lucideAnimated'
...
const rawFileStatus = computed(() => rawFile.value?.status || 'draft')
const isDragging = ref(false)

const { gridSize, viewMode, showFilename, sortOrder } = storeToRefs(rawFileStore)
...
const loadRawFile = async () => {
  ...
  try {
    ...
    await mediaSetsSidebar.setContext(rawFileId, null)

    // Wait for store state to update
    await nextTick()

    // Determine which set to select: route query > store auto-rawFile > first set
...
watch(
  ...
  setIdFromRoute => {
    if (!setIdFromRoute) {
      // If setId is removed from route, clear rawFile
    ...
    // Only update if different from current rawFile
...
const isPhotosTabActive = computed(() => {
  ...
  return (
    tab === 'photos' ||
    routeName === 'rawFilePhotos' ||
    routeName === 'rawFilePreview' ||
    (!tab && routeName !== 'rawFileSettings')
  )
})
...
// Watch route params to reload when rawFile ID changes
// Watch for route changes to handle navigation between different rawFiles
watch(
  () => route.params.id,
  rawFileId => {
    if (rawFileId) {
...
// Watch for modal opening to load rawFiles
watch(showMoveCopyModal, isOpen => {
  if (isOpen) {
    // Only load rawFiles when modal opens
    if (availableRawFiles.value.length === 0) {
      loadAvailableRawFiles()
    }
    // Initialize with current rawFile
    if (rawFile.value) {
      targetRawFileId.value = rawFile.value.id
      handleTargetRawFileChange(rawFile.value.id)
    }
  }
})
...
watch(
  ...
  id => {
    ...
    try {
      const raw = sessionStorage.getItem(SET_LIMIT_BANNER_STORAGE_KEY)
      const set = new Set(raw ? JSON.parse(raw) : [])
      setLimitBannerDismissed.value = set.has(`rawfile_${id}`)
    } catch (_) {
...
const fetchSetMedia = async params => {
  ...
  const response = await rawFilesApi.fetchSetMedia(rawFile.value.id, setId, fetchParams)
...
const {
  ...
  initialPage: 1,
  initialPerPage: 10,
  autoFetch: false, // We'll call fetch manually when rawFile/set changes
...
watch(
  ...
  async (newSetId, oldSetId) => {
    // Skip if this is the initial load and we're already loading in loadRawFile
    if (isInitialLoad && oldSetId === undefined) {
      isInitialLoad = false
      return
    }

    if (rawFile.value?.id && newSetId) {
  ...
  { immediate: false } // Don't trigger on initial load, loadRawFile handles it
)

// Initialize rawFile workflow for uploads
const {
  processFiles,
  uploadMediaToSet,
...
watch(isUploadingFromWorkflow, async (val, oldVal) => {
  ...
  if (!val && oldVal) {
    ...
    // Note: Media sets are now reloaded via onUploadComplete callback in useRawFileWorkflow
...
watch(
  sortOrder,
  () => {
    if (rawFile.value?.id && selectedSetId.value && !isUploading.value && !isLoadingMedia.value) {
      loadMediaItems()
    }
  },
  { immediate: false }
...
// Check if selected items can be previewed
const canPreviewSelected = computed(() => {
  if (selectedMediaIds.value.size === 0) return false
  const selectedItems = sortedMediaItems.value.filter(item =>
    selectedMediaIds.value.has(getItemId(item))
  )
  return selectedItems.some(item => canPreviewFile(item?.file || item))
})
...
const handleToggleRecommended = async item => {
  if (!item?.id && !item?.uuid || !rawFile.value?.id || !selectedSetId.value) {
  ...
  try {
    ...
    const response = await rawFilesApi.toggleRecommended(
      rawFile.value.id,
      selectedSetId.value,
      mediaId
    )
...
// Use rawFile actions composable
const rawFileId = computed(() => rawFile.value?.id)
const { copyFilenames, resetRawFileLimit, isCopyingFilenames, isResettingLimit } =
  useRawFileActions(rawFileId)
...
const {
  ...
  selectedWatermark: computed(() => rawFile.value?.watermarkId || 'none'),
  isApplyingWatermark,
  watermarkStore,
  mediaApi: null,
  rawFilesApi,
...
const {
  ...
  api: rawFilesApi,
  context: rawFile,
  contextId: computed(() => rawFile.value?.id),
  setId: selectedSetId,
  mediaItems,
  selectedMediaIds,
  sortedMediaItems,
...
// Use rawFile progress composable
const {
...
// Override overallProgress for completed rawFiles to use rawFile's stored counts
const overallProgress = computed(() => {
  // For completed rawFiles, use the rawFile's stored counts (stable values)
  if (rawFile.value?.status === 'completed') {
    // Try different property names that might come from the backend
    const selected =
      rawFile.value.selected_count ||
      rawFile.value.selectedCount ||
      rawFile.value.selectedMediaCount ||
      0
    const total = rawFile.value.media_count || rawFile.value.mediaCount || 0
  ...
  // For active/draft rawFiles, use the composable's progress
...
// Count of items with isSelected: true across entire rawFile
// Use rawFile's stored count from backend for accuracy (includes all sets, not just loaded ones)
const selectedCountAcrossRawFile = computed(() => {
  // For completed rawFiles, use the rawFile's stored count
  if (rawFile.value?.status === 'completed') {
    return (
      rawFile.value.selected_count ||
      rawFile.value.selectedCount ||
  ...
  // For active/draft rawFiles, count from loaded media items
...
const handleCopySelectedFilenamesInSet = async setId => {
  ...
  // For completed rawFiles, use the backend API to get selected filenames for the set
  if (rawFile.value?.status === 'completed') {
    try {
      const result = await rawFilesApi.getSelectedFilenames(rawFile.value.id, setId)
  ...
  // For active/draft rawFiles, use loaded media items
...
// Copy filenames of all items with isSelected: true across the entire rawFile
const handleCopyAllSelectedFilenames = async () => {
  // For completed rawFiles, use the backend API to get all selected filenames across all sets
  if (rawFile.value?.status === 'completed') {
    try {
      const result = await rawFilesApi.getSelectedFilenames(rawFile.value.id, null)
      const filenames = result.filenames || []

      if (filenames.length === 0) {
        toast.info('No selected items', {
          description: 'No items with selected status in this rawFile.',
        })
        return
      }
...
// Handle reset rawFile limit
const handleResetRawFileLimit = async () => {
  try {
    const updatedRawFile = await resetRawFileLimit()
    if (updatedRawFile) {
...
// RawFile limit modals state
const showRawFileLimitModal = ref(false)
const isSavingRawFileLimit = ref(false)

// Handle rawFile limit modal
const handleOpenRawFileLimitModal = () => {
  showRawFileLimitModal.value = true
}

const handleSaveRawFileLimit = async limit => {
  if (!rawFile.value?.id) return

  isSavingRawFileLimit.value = true
  try {
    const updatedRawFile = await rawFilesApi.updateRawFile(rawFile.value.id, {
      rawFileLimit: limit,
    })
    rawFile.value = updatedRawFile
    showRawFileLimitModal.value = false
  } catch (error) {
    toast.error('Failed to update rawFile limit', {
      description: error?.message || 'An unknown error occurred',
    })
  } finally {
    isSavingRawFileLimit.value = false
  }
}

const handleCancelRawFileLimit = () => {
  showRawFileLimitModal.value = false
}
...
// Load available rawFiles for move/copy modal
const loadAvailableRawFiles = async () => {
  isLoadingRawFiles.value = true
  try {
    const result = await rawFileStore.fetchAllRawFiles({ perPage: 100 })
...
const handleTargetRawFileChange = async rawFileId => {
  targetRawFileId.value = rawFileId
  targetSetId.value = '' // Reset set rawFile when rawFile changes

  if (!rawFileId) {
    targetRawFileSets.value = []
    return
  }
  ...
  try {
    ...
    if (rawFileId === rawFile.value?.id) {
      ...
      // Fetch sets from API for other rawFiles
      const sets = await rawFilesApi.fetchMediaSets(rawFileId)
      allSets = Array.isArray(sets) ? sets : []
    }

    // Filter out the current set to prevent moving/copying to the same set
    // Only filter if we're in the same rawFile (moving within the same rawFile)
    if (rawFileId === rawFile.value?.id && selectedSetId.value) {
      targetRawFileSets.value = allSets.filter(set => set.id !== selectedSetId.value)
    } else {
      targetRawFileSets.value = allSets
    }

    // Auto-select the first set if available and only one set exists
    if (targetRawFileSets.value.length === 1) {
      targetSetId.value = targetRawFileSets.value[0].id
    }
  } catch (error) {
    targetRawFileSets.value = []
...
const handleConfirmMoveCopy = async () => {
  // Validate inputs
  if (!targetRawFileId.value || selectedMediaIds.value.size === 0) {
    toast.error('Missing rawFile', {
      description: 'Please select a target rawFile.',
    })
    return
  }
  ...
  if (!rawFile.value?.id || !selectedSetId.value) {
    toast.error('Invalid context', {
      description: 'Unable to determine source rawFile or set.',
    })
    return
  }
  ...
  if (targetSet === selectedSetId.value && targetRawFileId.value === rawFile.value?.id) {
  ...
  try {
    if (moveCopyAction.value === 'move') {
      ...
      const result = await rawFilesApi.moveMediaToSet(
        rawFile.value.id,
        selectedSetId.value,
        mediaIds,
        targetSet
      ...
      if (targetRawFileId.value === rawFile.value?.id) {
        await mediaSetsSidebar.loadMediaSets()
      }
      ...
      const result = await rawFilesApi.copyMediaToSet(
        rawFile.value.id,
        selectedSetId.value,
        mediaIds,
        targetSet
      ...
      if (targetRawFileId.value === rawFile.value?.id && targetSet === selectedSetId.value) {
        await loadMediaItems()
      }
...
const handleToggleMediaRawFile = id => {
...
const handleSetAsCover = async item => {
  if (!rawFile.value?.id || !item?.id || isUpdatingCoverPhoto.value) return
  ...
  const existingFocalPoint = rawFile.value?.coverFocalPoint ||
    rawFile.value?.cover_focal_point || { x: 50, y: 50 }
...
const handleFocalPointConfirm = async focalPoint => {
  if (!rawFile.value?.id || !selectedMediaForCover.value?.id || isUpdatingCoverPhoto.value) return
  ...
  try {
    ...
    await rawFilesApi.setCoverPhotoFromMedia(
      rawFile.value.id,
    ...
    if (rawFile.value) {
      ...
      rawFile.value.coverPhotoUrl = coverUrl
      rawFile.value.cover_photo_url = coverUrl
      rawFile.value.coverFocalPoint = focalPoint
      rawFile.value.cover_focal_point = focalPoint
      // Force reactivity
...
const handleConfirmBulkWatermark = async () => {
  ...
  try {
    ...
    for (let i = 0; i < imageItems.length; i++) {
      ...
      try {
        const rawFileIdValue = rawFile.value?.id
        const setIdValue = selectedSetId.value

        if (rawFilesApi && rawFilesApi.applyWatermarkToMedia) {
          await rawFilesApi.applyWatermarkToMedia(
            rawFileIdValue,
            setIdValue,
            item.id,
...
const getDeleteModalWarning = () => {
  ...
  // For media items, show location (rawFile and set)
  if (item.collectionId || item.setId) {
    const locationParts = []

    if (rawFile.value?.name) {
      locationParts.push(`RawFile: ${rawFile.value.name}`)
    }
...
const handleAddMedia = () => {
  if (rawFile.value?.status === 'completed') {
    toast.info('Upload disabled', {
      description: 'Cannot upload media to a completed rawFile.',
    })
    return
  }
...
const handleRetryUpload = async (fileId, retryFn) => {
  ...
  if (!rawFile.value?.id || !selectedSetId.value) {
    toast.error('Invalid context', {
      description: 'RawFile or set not found.',
    })
    return
  }
...
// Download PIN helpers and handlers
const requiresDownloadPin = computed(() => {
  return rawFile.value?.download?.downloadPinEnabled ?? false
})

const getStoredDownloadPin = (rawFileId) => {
  const stored = localStorage.getItem(`raw_file_download_pin_${rawFileId}`)
  if (stored) {
    try {
      ...
      localStorage.removeItem(`raw_file_download_pin_${rawFileId}`)
    } catch (e) {
      localStorage.removeItem(`raw_file_download_pin_${rawFileId}`)
    }
  }
...
const storeDownloadPin = (rawFileId, pin) => {
  const expiresAt = Date.now() + 15 * 60 * 1000 // 15 minutes
  localStorage.setItem(`raw_file_download_pin_${rawFileId}`, JSON.stringify({ pin, expiresAt }))
  ...
  try {
    ...
    if (verified) {
      ...
      if (pendingDownloadItem.value) {
        ...
        await handleDownloadMedia(item, verifiedDownloadPin.value)
      }
    } else {
      downloadPinError.value = response.message || 'Incorrect PIN. Please try again.'
      downloadPinInput.value = ''
    }
  } catch (err) {
    downloadPinError.value = err.message || err?.response?.data?.message || 'Failed to verify PIN. Please try again.'
    downloadPinInput.value = ''
  } finally {
...
// Wrapper for handleDownloadMedia with download PIN check
const handleDownloadMediaWithPin = async (item) => {
  if (requiresDownloadPin.value) {
    const rawFileId = rawFile.value?.id
    if (!rawFileId) {
      toast.error('Raw file not found')
      return
    }
    
    // Check if PIN is already verified and stored
    const storedPin = getStoredDownloadPin(rawFileId)
    if (storedPin) {
      verifiedDownloadPin.value = storedPin
      isDownloadPinVerified.value = true
      await handleDownloadMedia(item, storedPin)
      return
    }
    
    // Show download PIN modal
    if (!isDownloadPinVerified.value) {
      pendingDownloadItem.value = item
      showDownloadPinModal.value = true
      return
    }
    
    // PIN is verified, proceed with download
    await handleDownloadMedia(item, verifiedDownloadPin.value)
  } else {
    // No download PIN required, proceed normally
