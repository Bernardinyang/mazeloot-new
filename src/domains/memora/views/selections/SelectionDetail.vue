...
watch(
  ...
  id => {
    ...
    try {
      const raw = sessionStorage.getItem(SET_LIMIT_BANNER_STORAGE_KEY)
      const set = new Set(raw ? JSON.parse(raw) : [])
      setLimitBannerDismissed.value = set.has(`selection_${id}`)
    } catch (_) {
...
const {
  ...
  // Refresh set counts and media list only; do not refetch full selection to avoid page reload
