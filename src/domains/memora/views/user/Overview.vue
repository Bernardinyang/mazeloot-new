...
// Selected Products
const mazelootProducts = ref([])

const handleAppSwitch = product => {
  // Use new route format with productSlug
  if (product.slug) {
    navigateTo({ path: `/${product.slug}/dashboard` })
...
const handleProductClick = product => {
  ...
  if (product.slug === 'memora') {
    navigateTo({ path: '/memora/dashboard' })
    return
  }
...
const fetchProductPreferences = async () => {
  try {
    ...
    // Extract product objects from preferences
    mazelootProducts.value = preferences
      .map(pref => pref.product)
      .filter(Boolean)
  } catch (error) {
    console.error('Failed to fetch product preferences:', error)
    // On error, show empty array (no products)
    mazelootProducts.value = []
  } finally {
