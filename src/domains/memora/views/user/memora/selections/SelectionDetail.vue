...
import SelectionSettingsGeneral from '@/domains/memora/views/user/memora/selections/settings/General.vue'
...
import { useSelectionStore } from '@/shared/stores/selection.js'
import { useSelectionMediaSetsSidebarStore } from '@/shared/stores/selectionMediaSetsSidebar'
...
import { useSelectionWorkflow } from '@/shared/composables/useSelectionWorkflow'
import { useSelectionActions } from '@/shared/composables/useSelectionActions'
import { useSelectionProgress } from '@/shared/composables/useSelectionProgress'
...
const handleToggleRecommended = async item => {
  ...
  if (!selection.value?.id || !selectedSetId.value) {
    toast.error('Invalid selection context')
    return
  }

  if (selection.value?.status === 'completed') {
    toast.error('Cannot mark as recommended in completed selections')
    return
  }

  const mediaId = item.id || item.uuid
  const isCurrentlyRecommended = item.isCreativeSelected || item.is_creative_selected || false

  try {
    toast.loading(isCurrentlyRecommended ? 'Removing recommendation...' : 'Marking as recommended...', {
      id: 'toggle-recommended',
    })

    const updatedMedia = await selectionsApi.toggleCreativeSelected(
      selection.value.id,
      selectedSetId.value,
      mediaId
    )
    ...
    if (mediaIndex !== -1) {
      mediaItems.value[mediaIndex] = {
        ...mediaItems.value[mediaIndex],
        isCreativeSelected: updatedMedia.isCreativeSelected ?? !isCurrentlyRecommended,
        is_creative_selected: updatedMedia.isCreativeSelected ?? !isCurrentlyRecommended,
      }
    }

    toast.success(updatedMedia.isCreativeSelected ? 'Marked as recommended' : 'Removed recommendation', {
      id: 'toggle-recommended',
    })
  } catch (error) {
