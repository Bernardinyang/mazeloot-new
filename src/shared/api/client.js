...
class ApiClient {
  ...
  async upload(endpoint, file, options) {
    const { skipAuth = false, phase, purpose, ...fetchOptions } = options || {}
  ...
  async uploadMultiple(endpoint, files, options = {}) {
    const { skipAuth = false, onProgress, signal, phase, purpose, ...fetchOptions } = options || {}
    ...
    if (onProgress || signal) {
      return new Promise((resolve, reject) => {
        ...
        // Append phase/purpose if provided
        if (phase) {
          formData.append('phase', phase)
        }
        if (purpose) {
  ...
  async uploadWithProgress(endpoint, file, options = {}) {
    const { skipAuth = false, onProgress, signal, phase, purpose } = options
