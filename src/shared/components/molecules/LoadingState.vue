<template>
  <div v-if="variant === 'skeleton'" :class="['py-12', containerClass]">
    <slot name="skeleton">
      <CardGridSkeleton :count="skeletonCount" />
    </slot>
  </div>
  <div v-else :class="['flex flex-col items-center justify-center py-12', containerClass]">
...
import CardGridSkeleton from '@/shared/components/molecules/CardGridSkeleton.vue'

const props = defineProps({
  message: { type: String, default: 'Loading...' },
  variant: { type: String, default: 'spinner', validator: v => ['spinner', 'skeleton'].includes(v) },
