<template>
  <div class="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-4">
    ...
    <div
      class="flex flex-col sm:flex-row items-start sm:items-center gap-3 sm:gap-4 flex-1 min-w-0 w-full sm:w-auto"
    >
      <h1
        :class="theme.textPrimary"
        class="text-2xl sm:text-3xl md:text-4xl font-bold tracking-tight shrink-0"
      >
        {{ title }}
      </h1>
    ...
    <!-- Right: Actions -->
    <div
      class="flex items-center gap-2 sm:gap-3 flex-wrap sm:flex-nowrap shrink-0 w-full sm:w-auto justify-start sm:justify-end"
    >
      <slot name="actions" />
      ...
      <DropdownMenu v-if="showSort && sortOptions.length > 0">
        <DropdownMenuTrigger as-child>
          <Button
            :class="[theme.textSecondary, theme.bgButtonHover, 'gap-1 sm:gap-2']"
            size="sm"
            variant="ghost"
          >
      ...
      <DropdownMenu v-if="showViewToggle">
        ...
        <DropdownMenuContent
          :class="[
            theme.bgDropdown,
            theme.borderSecondary,
            'w-[calc(100vw-2rem)] sm:min-w-[160px] sm:w-auto',
          ]"
          align="end"
        >
