<template>
  <SidebarModal
    ...
    title="New Raw Files Phase"
    @update:model-value="$emit('update:open', $event)"
  >
    <form id="raw-files-form" class="space-y-5" @submit.prevent="handleSubmit">
      <div class="space-y-2">
...
const getExistingColors = () => {
  return props.existingRawFiles.map(rf => rf.color).filter(Boolean)
}
...
const handleSubmit = async () => {
  ...
  try {
    const newRawFiles = await rawFilesApi.createRawFiles({
    ...
    toast.success('Raw files phase created', {
      description: 'Your new raw files phase has been created.',
    })

    emit('update:open', false)
    emit('success', newRawFiles)
  } catch (error) {
    await handleError(error, {
      fallbackMessage: 'Failed to create raw files phase.',
    })
  } finally {
