<template>
  <Dialog :open="isOpen" @update:open="isOpen = $event">
    <DialogContent :class="[theme.bgCard, theme.borderSecondary, 'sm:max-w-[520px]']" class="!fixed !left-1/2 !top-1/2 !-translate-x-1/2 !-translate-y-1/2">
      <DialogHeader>
        ...
        <DialogDescription :class="theme.textSecondary" class="text-sm mt-1">
          Share your homepage with others via link, QR code, WhatsApp, or email.
        </DialogDescription>
      </DialogHeader>
      ...
      <div class="space-y-6 mt-6">
        <!-- Share Link Section -->
        <div class="space-y-2">
          <label :class="theme.textPrimary" class="text-sm font-semibold"> Homepage Link </label>
        ...
        <div v-if="homepagePassword" class="space-y-2">
          ...
          <p :class="theme.textSecondary" class="text-xs">
            Share this password with visitors to access your homepage
          </p>
        </div>
        ...
        <div class="space-y-2">
          ...
          >
            ...
            <p :class="theme.textSecondary" class="text-xs text-center max-w-xs">
              Scan this QR code to access the homepage
            </p>
          </div>
        </div>
      ...
      <DialogFooter class="mt-6">
        <Button variant="ghost" :class="[theme.textSecondary, theme.bgButtonHover]" @click="close">
          Close
        </Button>
      </DialogFooter>
    </DialogContent>
...
const generateQRCode = async () => {
  try {
    const qrUrl = `https://api.qrserver.com/v1/create-qr-code/?size=200x200&data=${encodeURIComponent(props.homepageUrl)}`
...
const handleCopyLink = async () => {
  try {
    await navigator.clipboard.writeText(props.homepageUrl)
    toast.success('Link copied', {
      description: 'Homepage link has been copied to clipboard.',
    })
  } catch (error) {
...
const handleCopyPassword = async () => {
  if (!props.homepagePassword) return
  try {
    await navigator.clipboard.writeText(props.homepagePassword)
...
const handleDownloadQR = () => {
  ...
  try {
    ...
    link.download = `homepage-qr-code.png`
...
const handleShareWhatsApp = () => {
  const brandName = props.brandName || 'My Homepage'
  const bio = props.biography ? `\n\n${props.biography.substring(0, 100)}${props.biography.length > 100 ? '...' : ''}` : ''
  const message = encodeURIComponent(`Check out ${brandName}${bio}\n\n${props.homepageUrl}`)
...
const handleShareEmail = () => {
  const brandName = props.brandName || 'My Homepage'
  const bio = props.biography ? `\n\n${props.biography}` : ''
  const subject = encodeURIComponent(`Check out ${brandName}`)
  const body = encodeURIComponent(`Check out ${brandName}${bio}\n\nVisit: ${props.homepageUrl}${props.homepagePassword ? `\n\nPassword: ${props.homepagePassword}` : ''}`)
