  ...
  >
    ...
    <div v-else-if="media" class="space-y-6">
      ...
      <DetailSection v-if="hasRelationships" title="Relationships" container-class="pt-4 border-t">
        ...
        <DetailField v-if="media.rawFilesId || media.raw_files_uuid" label="Raw Files ID" format="mono">
          <div class="flex items-center gap-2">
            <code
              :class="[
                'text-xs px-2 py-1 rounded',
            ...
            >
              {{ media.rawFilesId || media.raw_files_uuid }}
            </code>
            <button
              class="p-1 rounded hover:bg-gray-100 dark:hover:bg-gray-800 transition-colors"
              :class="theme.textSecondary"
              @click="copyToClipboard(media.rawFilesId || media.raw_files_uuid, 'Raw Files ID')"
              title="Copy Raw Files ID"
            >
              <Copy class="h-3.5 w-3.5" />
            </button>
          </div>
...
import { getMediaDisplayUrl, getMediaDisplayUrlSync } from '@/shared/utils/media/getMediaDisplayUrl'
...
const isRawFilesContext = computed(() => {
  if (!props.media) return false
  return !!(props.media.rawFilesId || props.media.raw_files_uuid)
})
...
const hasRelationships = computed(() => {
  ...
  return (
    ...
    props.media.rawFilesId ||
    props.media.raw_files_uuid ||
...
const isSelectionContext = computed(() => {
  if (!props.media) return false
  // Don't show selection context for raw files phase
  if (isRawFilesContext.value) return false
...
const handleDownload = async () => {
  ...
  try {
    ...
    const { blob, filename } = isRawFilesContext.value
      ? await rawFilesApi.downloadMedia(props.media.id)
      : await selectionsApi.downloadMedia(props.media.id)
