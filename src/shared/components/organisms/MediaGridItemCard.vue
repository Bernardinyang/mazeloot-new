  <template>
    ...
    >
      ...
      <div
        ...
        :class="canOpenInLightbox ? 'cursor-pointer' : 'cursor-default'"
        @click="canOpenInLightbox ? emit('open-viewer', props.item) : null"
      >
        <!-- Image thumbnail -->
        <img
          v-if="(item?.type || item?.file?.type) !== 'video' && !hasImageError && isImageLoaded"
          ref="imageElementRef"
        ...
        />
        <!-- Hidden image to trigger load/error events for non-image files -->
        <img
          v-if="(item?.type || item?.file?.type) !== 'video' && !hasImageError && !isImageLoaded && imageSrc && imageSrc !== placeholderImage && canOpenInLightbox"
          ref="imageElementRef"
          :alt="props.item?.filename || 'Media'"
          class="hidden"
          :src="imageSrc"
          ...
          @load="handleImageLoad"
        />
        <!-- Skeleton loader while image is loading (only for files that can be displayed) -->
        <div
          v-if="(item?.type || item?.file?.type) !== 'video' && !hasImageError && !isImageLoaded && imageSrc && imageSrc !== placeholderImage && canOpenInLightbox"
          class="w-full h-full bg-gradient-to-br from-gray-200 via-gray-100 to-gray-200 dark:from-gray-800 dark:via-gray-700 dark:to-gray-800 animate-pulse"
        />
        <!-- File type icon fallback when image fails to load or for non-displayable files -->
        <div
          v-if="(hasImageError || shouldShowFileIcon) && (item?.type || item?.file?.type) !== 'video'"
          class="w-full h-full flex items-center justify-center bg-gray-100 dark:bg-gray-800"
        >
          <component
            :is="fileTypeIconComponent"
            :class="[
              'transition-all duration-300',
              fileTypeCategory === 'raw' ? 'text-orange-500' :
              fileTypeCategory === 'image' ? 'text-blue-500' :
              fileTypeCategory === 'video' ? 'text-purple-500' :
              fileTypeCategory === 'audio' ? 'text-green-500' :
              fileTypeCategory === 'document' ? 'text-red-500' :
              fileTypeCategory === 'archive' ? 'text-yellow-500' :
              'text-gray-500',
            ]"
            :size="48"
          />
        ...
        >
          <img
            v-if="imageSrc && imageSrc !== placeholderImage && !hasImageError && isImageLoaded"
          ...
          <img
            ...
            @load="handleImageLoad"
          />
          <!-- Skeleton loader while video thumbnail is loading -->
          <div
            v-if="imageSrc && imageSrc !== placeholderImage && !hasImageError && !isImageLoaded"
            class="w-full h-full bg-gradient-to-br from-gray-200 via-gray-100 to-gray-200 dark:from-gray-800 dark:via-gray-700 dark:to-gray-800 animate-pulse"
          />
          <!-- File type icon fallback when video thumbnail fails to load -->
          <div
            v-if="hasImageError || (imageSrc === placeholderImage || !imageSrc)"
            class="w-full h-full flex items-center justify-center bg-gray-100 dark:bg-gray-800"
          >
            <component
              :is="fileTypeIconComponent"
      ...
      <div class="absolute bottom-2 left-2 z-30 flex items-center gap-1.5">
        ...
        <!-- Recommended Badge (only in selection phase) -->
        <!-- #region agent log -->
        <div v-if="false" style="display:none">{{ (() => { fetch('http://127.0.0.1:7242/ingest/0c1f4b73-4437-4f70-a7a0-856984785a37',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({location:'MediaGridItemCard.vue:338',message:'badge condition check',data:{phaseType:phaseType,propsPhaseType:props.phaseType,isCreativeSelected:props.item?.isCreativeSelected,condition:phaseType === 'selection'},timestamp:Date.now(),sessionId:'debug-session',runId:'run1',hypothesisId:'C'})}).catch(()=>{}); return ''; })() }}</div>
        <!-- #endregion -->
        <div v-if="props.phaseType === 'selection' && props.item?.isCreativeSelected" class="flex items-center justify-center">
          <div
            class="flex items-center justify-center w-8 h-8 rounded-full bg-blue-500/90 dark:bg-blue-600/90 backdrop-blur-sm shadow-lg"
            title="Recommended"
          >
      ...
      >
        ...
        <div class="relative flex items-center gap-2.5 px-4 py-3 bg-white/10 dark:bg-gray-900/10 backdrop-blur-md rounded-full border border-white/20 dark:border-gray-700/20 shadow-xl transform transition-all duration-300 ease-out group-hover:scale-100 scale-95">
          ...
          <TooltipProvider v-if="!props.hideFavoriteIcon && props.phaseType !== 'raw_files'">
            <Tooltip>
              <TooltipTrigger as-child>
                <button
                  class="flex items-center justify-center w-10 h-10 rounded-full bg-black/80 hover:bg-black dark:bg-gray-900/90 dark:hover:bg-gray-800 transition-all duration-300 ease-out shadow-md hover:shadow-xl hover:scale-110 active:scale-95 transform hover:-translate-y-0.5"
      ...
      >
        <DropdownMenu v-model:open="isDropdownOpen">
          ...
          >
            ...
            <DropdownMenuItem
              v-if="props.phaseType === 'raw_files' || (props.allowDownload && isCollectionContext)"
            ...
            <DropdownMenuItem
              v-if="props.phaseType !== 'raw_files'"
              :class="[theme.textPrimary, theme.bgButtonHover, 'cursor-pointer']"
            ...
            <!-- #region agent log -->
            <div v-if="false">{{ (() => { fetch('http://127.0.0.1:7242/ingest/0c1f4b73-4437-4f70-a7a0-856984785a37',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({location:'MediaGridItemCard.vue:539',message:'dropdown condition check',data:{phaseType:phaseType,propsPhaseType:props.phaseType,condition:phaseType === 'selection'},timestamp:Date.now(),sessionId:'debug-session',runId:'run1',hypothesisId:'D'})}).catch(()=>{}); return ''; })() }}</div>
            <!-- #endregion -->
            <DropdownMenuItem
              v-if="props.phaseType === 'selection'"
              :class="[theme.textPrimary, theme.bgButtonHover, 'cursor-pointer']"
              @select.prevent="emit('toggle-recommended')"
            >
              <ThumbsUp :class="['h-4 w-4 mr-2', props.item?.isCreativeSelected ? 'fill-blue-400 text-blue-400' : '']" />
              {{ props.item?.isCreativeSelected ? 'Remove Recommendation' : 'Mark as Recommended' }}
            </DropdownMenuItem>
            ...
            <template v-if="props.showManagementActions">
              ...
              <DropdownMenuItem
                v-if="!props.minimalActions && props.phaseType !== 'raw_files'"
                :class="[theme.textPrimary, theme.bgButtonHover, 'cursor-pointer']"
              ...
              <DropdownMenuItem
                v-if="!props.minimalActions && !(props.item?.isCompleted || props.item?.is_completed) && props.phaseType !== 'raw_files'"
                :class="[theme.textPrimary, theme.bgButtonHover, 'cursor-pointer']"
              ...
              <DropdownMenuItem
                ...
                @click.stop="emit('star-click')"
              >
                <Star :class="['h-4 w-4 mr-2', props.item?.isStarred ? 'fill-yellow-400 text-yellow-400' : '']" />
                {{ props.item?.isStarred ? 'Unstar' : 'Star' }}
              </DropdownMenuItem>
            </template>
...
import {
  ...
  revokeMediaBlobUrl,
} from '@/shared/utils/media/getMediaDisplayUrl'
import { getFileTypeIcon, getFileTypeCategory } from '@/shared/utils/media/getFileTypeIcon'
import {
  CheckCircle2,
  CheckSquare2,
  Lock,
...
// #region agent log
watch(() => props.phaseType, (newVal, oldVal) => {
  fetch('http://127.0.0.1:7242/ingest/0c1f4b73-4437-4f70-a7a0-856984785a37',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({location:'MediaGridItemCard.vue:953',message:'props phaseType changed',data:{phaseType:props.phaseType,newVal,oldVal,allProps:Object.keys(props)},timestamp:Date.now(),sessionId:'debug-session',runId:'run3',hypothesisId:'F'})}).catch(e=>console.error('Log error:',e));
}, { immediate: true });
onMounted(() => {
  fetch('http://127.0.0.1:7242/ingest/0c1f4b73-4437-4f70-a7a0-856984785a37',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({location:'MediaGridItemCard.vue:onMounted',message:'props received on mount',data:{phaseType:props.phaseType,allProps:Object.keys(props),hasPhaseType:'phaseType' in props},timestamp:Date.now(),sessionId:'debug-session',runId:'run3',hypothesisId:'A'})}).catch(e=>console.error('Log error:',e));
});
fetch('http://127.0.0.1:7242/ingest/0c1f4b73-4437-4f70-a7a0-856984785a37',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({location:'MediaGridItemCard.vue:956',message:'props received sync',data:{phaseType:props.phaseType,allProps:Object.keys(props),hasPhaseType:'phaseType' in props},timestamp:Date.now(),sessionId:'debug-session',runId:'run3',hypothesisId:'A'})}).catch(e=>console.error('Log error:',e));
// #endregion
...
// #region agent log
onMounted(() => {
  fetch('http://127.0.0.1:7242/ingest/0c1f4b73-4437-4f70-a7a0-856984785a37',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({location:'MediaGridItemCard.vue:onMounted',message:'props received on mount',data:{phaseType:props.phaseType,allProps:Object.keys(props),hasPhaseType:'phaseType' in props},timestamp:Date.now(),sessionId:'debug-session',runId:'run2',hypothesisId:'A'})}).catch(e=>console.error('Log error:',e));
});
// #endregion
...
// File type icon for fallback
const fileTypeIcon = computed(() => {
  const mimeType = props.item?.file?.mime_type || props.item?.file?.type || props.item?.type || ''
  const filename = props.item?.filename || props.item?.title || ''
  return getFileTypeIcon(mimeType, filename)
})

const fileTypeCategory = computed(() => {
  const mimeType = props.item?.file?.mime_type || props.item?.file?.type || props.item?.type || ''
  const filename = props.item?.filename || props.item?.title || ''
  return getFileTypeCategory(mimeType, filename)
})

const fileTypeIconComponent = computed(() => {
  const iconMap = {
    Camera,
    Image: ImageIcon,
    Video,
  ...
  return iconMap[fileTypeIcon.value] || File
})

// Check if file can be opened in MediaLightbox (only images and videos)
const canOpenInLightbox = computed(() => {
  const mediaType = props.item?.type || props.item?.file?.type
...
const updateImageSrc = async () => {
  ...
  if (!url) {
    ...
    // For non-image files, don't wait for load
    if (!canOpenInLightbox.value) {
      hasImageError.value = true
    }
    return
  }
  ...
  try {
    ...
    // Set timeout to show file type icon if image doesn't load within 3 seconds
    if (imageLoadTimeout) {
      clearTimeout(imageLoadTimeout)
    }
    imageLoadTimeout = setTimeout(() => {
      if (!isImageLoaded.value && !hasImageError.value) {
        hasImageError.value = true
      }
    }, 3000)
  } catch (error) {
    imageSrc.value = props.placeholderImage
    hasImageError.value = false
    // For non-image files, show file type icon immediately
    if (!canOpenInLightbox.value) {
      hasImageError.value = true
    }
  }
