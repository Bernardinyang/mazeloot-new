...
import { getMediaDisplayUrl } from '@/shared/utils/media/getMediaDisplayUrl'
...
const isCollectionContext = computed(() => {
  return props.collectionId && !props.proofingId && !props.selectionId && !props.rawFilesId
})
...
watch(isOpen, open => {
  if (open) {
    ...
    if (props.publicMode && isPrivate.value && props.collectionId && currentItem.value?.id) {
      import('@/domains/memora/api/collections').then(({ useCollectionsApi }) => {
