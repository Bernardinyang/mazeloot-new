<template>
  <div
    :class="[
      ...
      !props.minimalActions && hasRejectedClosureRequest
        ...
        : '',
      !props.minimalActions && hasPendingApprovalRequest
        ? 'ring-1 ring-blue-500 border-blue-500 bg-blue-50/30 dark:bg-blue-900/10 animate-pulse'
        : '',
      !props.minimalActions && isRejected ? 'ring-2 ring-red-600 border border-red-600 bg-red-50/30 dark:bg-red-900/10' : '',
  ...
  >
    ...
    <div
      class="flex items-center gap-4 flex-1 relative"
      :class="canOpenInLightbox ? 'cursor-pointer' : 'cursor-default'"
      @click="canOpenInLightbox ? emit('open-viewer') : null"
    >
      <div class="relative">
        <!-- Image thumbnail -->
        <img
          v-if="(props.item?.type || props.item?.file?.type) !== 'video' && !hasImageError && isImageLoaded"
          ref="imageElementRef"
          ...
          @load="() => { if (imageLoadTimeout) { clearTimeout(imageLoadTimeout); imageLoadTimeout = null; } isImageLoaded = true }"
        />
        <!-- Hidden image to trigger load/error events for non-image files -->
        <img
          v-if="(props.item?.type || props.item?.file?.type) !== 'video' && !hasImageError && !isImageLoaded && imageSrc && imageSrc !== placeholderImage && canOpenInLightbox"
          ref="imageElementRef"
          :alt="props.item?.filename || 'Media'"
          class="hidden"
          :src="imageSrc"
        ...
        <div
          v-if="(props.item?.type || props.item?.file?.type) !== 'video' && !hasImageError && !isImageLoaded && imageSrc && imageSrc !== placeholderImage && canOpenInLightbox"
          class="w-20 h-20 rounded-lg bg-gradient-to-br from-gray-200 via-gray-100 to-gray-200 dark:from-gray-800 dark:via-gray-700 dark:to-gray-800 animate-pulse"
        />
        <!-- File type icon fallback when image fails to load or for non-displayable files -->
        <div
          v-if="(hasImageError || shouldShowFileIcon) && (props.item?.type || props.item?.file?.type) !== 'video'"
          class="w-20 h-20 flex items-center justify-center bg-gray-100 dark:bg-gray-800 rounded-lg"
        >
          <component
        ...
        <div v-else class="w-20 h-20 relative rounded-lg overflow-hidden">
          ...
          <div
            v-if="imageSrc && imageSrc !== placeholderImage && !hasImageError && !isImageLoaded"
            class="w-full h-full rounded-lg bg-gradient-to-br from-gray-200 via-gray-100 to-gray-200 dark:from-gray-800 dark:via-gray-700 dark:to-gray-800 animate-pulse"
          />
          <!-- File type icon fallback when video thumbnail fails to load or for non-displayable files -->
          <div
            v-if="hasImageError || shouldShowFileIcon || (imageSrc === placeholderImage || !imageSrc)"
      ...
      <div class="flex-1 min-w-0">
        <div class="flex items-center gap-2">
          ...
          />
          <Lock
            v-if="props.item?.isPrivate || props.item?.is_private"
            :class="['h-3.5 w-3.5 shrink-0', theme.textSecondary]"
            title="Private"
          />
          <Sparkles
          ...
          />
          <ThumbsUp
            v-if="props.phaseType === 'selection' && props.item?.isCreativeSelected"
            class="h-3.5 w-3.5 shrink-0 fill-blue-500 text-blue-500"
            title="Recommended"
          />
        </div>
    ...
    <template v-if="!props.minimalActions && !props.selectionStatus">
    ...
    >
      <DropdownMenu v-model:open="isDropdownOpen">
        ...
        >
          ...
          <DropdownMenuItem
            v-if="props.phaseType === 'raw_files' || isCollectionContext"
            :class="[theme.textPrimary, theme.bgButtonHover, 'cursor-pointer']"
...
// Check if file type should show file icon immediately (non-displayable files)
const shouldShowFileIcon = computed(() => {
  return !canOpenInLightbox.value
})
