<template>
  ...
  >
    ...
    >
      <!-- Raw Files Title Popover (at top) -->
      <Popover>
        <PopoverTrigger as-child>
          ...
          >
            ...
            >
              {{ props.rawFiles?.name?.charAt(0)?.toUpperCase() || 'R' }}
            </span>
          </button>
        </PopoverTrigger>
        <PopoverContent
        ...
        >
          ...
          <template v-else>
            <p :class="theme.textPrimary" class="text-sm font-semibold">
              {{ props.rawFiles?.name || 'Loading...' }}
            </p>
            <p
              v-if="props.rawFiles?.status"
              :class="theme.textTertiary"
              class="text-xs mt-1 capitalize"
            >
              {{ props.rawFiles.status }}
            </p>
          </template>
        </PopoverContent>
      </Popover>
...
<script setup>
import { Download, ChevronsLeft, ChevronsRight, ImageIcon, Loader2, Settings } from 'lucide-vue-next'
