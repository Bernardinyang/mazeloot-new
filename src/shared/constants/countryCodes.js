/**
 * Country calling codes (E.164) for phone inputs.
 * Source: ITU E.164 / common public lists. Normalized and sorted (US, CA, GB first, then A–Z by name).
 */
const RAW = [
  ...
  { name: 'Congo (DRC)', dial_code: '+243', code: 'CD' },
  ...
  { name: 'São Tomé and Príncipe', dial_code: '+239', code: 'ST' },
...
function normalizeCode(d) {
  if (d == null || d === '') return ''
  return String(d).replace(/\s/g, '').trim()
}

const withCode = RAW.filter((c) => normalizeCode(c.dial_code))
const byCode = new Map()
withCode.forEach((c) => {
  if (!byCode.has(c.code)) byCode.set(c.code, { name: c.name, dial_code: normalizeCode(c.dial_code), code: c.code })
})

const PREFER_FIRST = ['US', 'CA', 'GB']
const rest = [...byCode.values()].sort((a, b) => a.name.localeCompare(b.name))
const preferred = PREFER_FIRST.map((code) => byCode.get(code)).filter(Boolean)
const others = rest.filter((c) => !PREFER_FIRST.includes(c.code))

/** All countries with calling code: { value, label, code } for dropdowns. US, CA, GB first, then A–Z. */
export const countryCodes = [
  ...preferred.map((c) => ({ value: c.code, label: c.name, code: c.dial_code })),
  ...others.map((c) => ({ value: c.code, label: c.name, code: c.dial_code })),
]
