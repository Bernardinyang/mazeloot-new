...
/**
 * Shared state for the Proofing sidebar "Media Sets" panel.
 * - Used by proofing views and detail sidebar.
 * - Goal: remove Media Sets prop-drilling.
 *
 * NOTE: We intentionally keep types loose to avoid TS churn during refactors.
 */
export const useProofingMediaSetsSidebarStore = defineStore('proofingMediaSetsSidebar', () => {
  const proofingStore = useProofingStore()
  const proofingApi = useProofingApi()

  // Context
  const proofingId = ref('')
  const projectId = ref(null) // null for standalone, UUID for project-based
  ...
  const loadMediaSets = async () => {
    if (!proofingId.value) return
    try {
      const fetchedSets = await proofingApi.fetchMediaSets(proofingId.value, projectId.value)
  ...
  const saveSetName = async setId => {
    const set = mediaSets.value.find(s => s.id === setId)
    if (!set || !ensureProofingId()) return
  ...
  const confirmDeleteSet = async () => {
    ...
    try {
      ...
      await proofingApi.deleteMediaSet(proofingId.value, deletingId, projectId.value)
  ...
  const handleSetDrop = async payload => {
    ...
    try {
      ...
      await proofingApi.reorderMediaSets(proofingId.value, setIds, projectId.value)
    } catch (error) {
