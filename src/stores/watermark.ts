...
import type { Watermark, CreateWatermarkData, UpdateWatermarkData } from '@/api/watermarks'

export const useWatermarkStore = defineStore('watermark', () => {
  const watermarks = ref<Watermark[]>([])
  const currentWatermark = ref<Watermark | null>(null)
   ...
   */
  const fetchWatermark = async (id: string) => {
    isLoading.value = true
    error.value = null
    try {
   ...
   */
  const createWatermark = async (data: CreateWatermarkData) => {
    isLoading.value = true
    error.value = null

    // Create temporary watermark for optimistic update
    const tempId = `temp-${Date.now()}`
    const tempWatermark: Watermark = {
      id: tempId,
      ...data,
   ...
   */
  const updateWatermark = async (id: string, data: UpdateWatermarkData) => {
    isLoading.value = true
    error.value = null
